...
vms_gmail_bp = Blueprint('gmail', __name__)

@vms_gmail_bp.route('/authenticate', methods=['GET'])
def authenticate_gmail():
    """Authenticate with Gmail"""
    try:
        reader = GmailReader()
        service = reader.authenticate()
        return jsonify({"message": "Gmail authenticated successfully", "status": "success"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@vms_gmail_bp.route('/urls/today', methods=['GET'])
...
@vms_gmail_bp.route('/process/today', methods=['POST'])
def process_todays_requisitions():
    ...
    try:
        ...
        return jsonify({"message": "Today's requisitions processing started", "status": "started"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@vms_gmail_bp.route('/emails/recent', methods=['GET'])
